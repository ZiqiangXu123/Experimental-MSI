...
RUN_DIR=${RUN_DIR:-$RUN_BASE/e7-reference-$(date +%Y%m%d-%H%M%S)}
...
"$ROOT/scripts/run_python.sh" "$ROOT/exp07_security_availability.pyz" plan --config "$RUN_DIR/config.json" --output "$RUN_DIR" >/dev/null
E7_BLOCKS=$(wc -l < "$RUN_DIR/e7_blocks.jsonl")
E8_BLOCKS=$(wc -l < "$RUN_DIR/e8_blocks.jsonl")
E7_JOB=$(sbatch --parsable "${SBATCH_SITE_OPTIONS[@]}" --array="0-$((E7_BLOCKS-1))%${E7_ARRAY_CONCURRENCY:-8}" \
...
E7_BASE=${E7_JOB%%;*}
E8_JOB=$(sbatch --parsable "${SBATCH_SITE_OPTIONS[@]}" --array="0-$((E8_BLOCKS-1))%${E8_ARRAY_CONCURRENCY:-16}" \
...
E8_BASE=${E8_JOB%%;*}
ANALYZE_JOB=$(sbatch --parsable "${SBATCH_SITE_OPTIONS[@]}" --dependency="afterok:${E7_BASE}:${E8_BASE}" \
...
printf '%s\n' "$E7_JOB" > "$RUN_DIR/e7_job_id.txt"; printf '%s\n' "$E8_JOB" > "$RUN_DIR/e8_job_id.txt"; printf '%s\n' "$ANALYZE_JOB" > "$RUN_DIR/analyze_job_id.txt"
