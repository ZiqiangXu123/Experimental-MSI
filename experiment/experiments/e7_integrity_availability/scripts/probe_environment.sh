...
echo '=== module command and runtimes ===';
...
echo '=== scratch variables ==='; printf 'SLURM_TMPDIR=%s\nTMPDIR=%s\n' "${SLURM_TMPDIR:-}" "${TMPDIR:-}"
echo '=== resource limits ==='; ulimit -a 2>&1 || true
echo '=== artifact runtime self-test ==='; "$ROOT/scripts/run_python.sh" "$ROOT/exp07_security_availability.pyz" info 2>&1 || true
