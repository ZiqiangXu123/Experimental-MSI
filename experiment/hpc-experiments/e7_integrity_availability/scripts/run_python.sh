...
run_container() {
  ...
  [[ -n "$engine" ]] || { echo 'ERROR: EXP07_CONTAINER is set but Apptainer/Singularity is unavailable.' >&2; exit 72; }
  [[ -r "$EXP07_CONTAINER" ]] || { echo "ERROR: unreadable container: $EXP07_CONTAINER" >&2; exit 72; }
  local script_root
  ...
  exec "$engine" exec "${binds[@]}" --pwd "$PWD" "$EXP07_CONTAINER" python3 "$@"
}
...
[[ -n "${EXP07_CONTAINER:-}" ]] && run_container "$@"
...
Choose one option:
  ...
  export EXP07_CONTAINER=$HOME/containers/python-3.12-bookworm.sif
MSG
exit 69
