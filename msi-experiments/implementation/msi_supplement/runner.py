...
def run_suite(opts: dict) -> Path:
    ...
    for key in ('sizes','queries','trials'):
        if opts.get(key) is not None:
            cfg[key]=opts[key]
    cfg['setup_timeout_seconds']=opts.get('setup_timeout',900)
    cfg['query_socket_timeout_seconds']=30
    ...
    client = Client(provider_url, read_token(Path(token_path)), setup_timeout=cfg['setup_timeout_seconds']) if provider_url else None
    ...
    try:
        ...
        for job_index,(kind,n,payloads,dm,trial,mode) in enumerate(mode_jobs):
            ...
            if remote:
                ...
                client.close()
                print(f'Uploading and preparing {n} blocks on provider (setup socket timeout {cfg["setup_timeout_seconds"]}s)',flush=True)
