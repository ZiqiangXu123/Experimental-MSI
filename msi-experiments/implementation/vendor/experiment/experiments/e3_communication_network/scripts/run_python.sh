...
run_container() {
  ...
  if [[ -z "$engine" ]]; then
    echo "ERROR: EXP03_CONTAINER is set, but apptainer/singularity is unavailable" >&2
    exit 72
  fi
  if [[ ! -r "$EXP03_CONTAINER" ]]; then
    echo "ERROR: EXP03_CONTAINER is not readable: $EXP03_CONTAINER" >&2
    exit 72
  fi
  local script_root
...
No administrator installation is required. Use one user-level path:
  ...
  3. Use an uploaded rootless Apptainer/Singularity image:
       export EXP03_CONTAINER=$HOME/containers/python-3.12-bookworm.sif
MSG
exit 69
