...
run_container() {
  ...
  [[ -n "$engine" ]] || {
    echo 'ERROR: EXP05_CONTAINER is set but Apptainer/Singularity is unavailable.' >&2
    exit 72
  }
  [[ -r "$EXP05_CONTAINER" ]] || {
    echo "ERROR: unreadable container: $EXP05_CONTAINER" >&2
    exit 72
  }
  local script_root
  ...
  exec "$engine" exec "${binds[@]}" --pwd "$PWD" "$EXP05_CONTAINER" python3 "$@"
}
...
[[ -n "${EXP05_CONTAINER:-}" ]] && run_container "$@"
...
Choose one option:
  ...
  export EXP05_CONTAINER=$HOME/containers/python-3.12-bookworm.sif
MSG
exit 69
