...
run_container() {
  ...
  [[ -n "$engine" ]] || {
    echo 'ERROR: EXP06_CONTAINER is set but Apptainer/Singularity is unavailable.' >&2
    exit 72
  }
  [[ -r "$EXP06_CONTAINER" ]] || {
    echo "ERROR: unreadable container: $EXP06_CONTAINER" >&2
    exit 72
  }
  local script_root
  ...
  for candidate in "${SLURM_TMPDIR:-}" "${RUN_BASE:-}" "${RUN_DIR:-}" "${E6_NODE_LOCAL_BASE:-}" "${E6_SHARED_WORK_BASE:-}"; do
    [[ -n "$candidate" && -d "$candidate" ]] && binds+=(--bind "$candidate:$candidate")
  done
  exec "$engine" exec "${binds[@]}" --pwd "$PWD" "$EXP06_CONTAINER" python3 "$@"
}
...
if [[ -n "${PYTHON_MODULE:-}" ]]; then
  initialise_modules
  type module >/dev/null 2>&1 || { echo 'ERROR: module command unavailable.' >&2; exit 71; }
...
[[ -n "${EXP06_CONTAINER:-}" ]] && run_container "$@"
...
Choose one option:
  ...
  export EXP06_CONTAINER=$HOME/containers/python-3.12-bookworm.sif
MSG
exit 69
