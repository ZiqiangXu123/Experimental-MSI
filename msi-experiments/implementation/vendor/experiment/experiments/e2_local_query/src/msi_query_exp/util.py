...
def pin_to_single_cpu() -> dict[str, Any]:
    result: dict[str, Any] = {'supported': False, 'pinned': False}
    if os.environ.get('E2_DISABLE_PINNING') == '1':
        result['reason'] = 'disabled by E2_DISABLE_PINNING'
        return result
...
def system_metadata() -> dict[str, Any]:
    return {'timestamp_utc': time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime()), 'hostname': socket.gethostname(), 'platform': platform.platform(), 'machine': platform.machine(), 'cpu_model': _cpu_model(), 'logical_cpu_count': os.cpu_count(), 'python': platform.python_version(), 'python_implementation': platform.python_implementation(), 'pid': os.getpid(), 'slurm': {key: os.environ.get(key) for key in ('SLURM_JOB_ID', 'SLURM_ARRAY_JOB_ID', 'SLURM_ARRAY_TASK_ID', 'SLURM_JOB_NAME', 'SLURM_CPUS_PER_TASK', 'SLURM_JOB_CPUS_PER_NODE', 'SLURM_NODELIST', 'SLURM_MEM_PER_NODE', 'SLURM_MEM_PER_CPU', 'SLURM_JOB_PARTITION', 'SLURM_JOB_ACCOUNT', 'SLURM_QOS', 'SLURM_CPU_BIND', 'SLURM_CPU_BIND_LIST')}}
